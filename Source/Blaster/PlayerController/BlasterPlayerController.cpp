	...
	if (CharacterOverlay == nullptr)
	{
		if (BlasterHUD && BlasterHUD->CharacterOverlay)
		{
			...
			if (CharacterOverlay)
			{
				...
				// 应该在OnJoinMidgame里设置比分初始值，在GameState编写策略函数，注册需要同步的变量
				// 方便后续添加更多数值
				if (bInitializeTeamScores) InitTeamScores();
				if(bHideTeamScores|| !bShowTeamScores) HideTeamScores();// 如果不是团队比赛不会复制
			}
		}
	}
}
		...
		if (bHUDValid)
		{
			...
			if (BlasterGameState && BlasterPlayerState)
			{
				...
				else if (TopPlayers.Num() == 1)
				{
					...
					if(TopPlayers[0] == BlasterPlayerState)InfoTextString.Append(FString("(YOU)"));
				}
				else if (TopPlayers.Num() > 1)
				{
					...
					{
						FString strIfLocal = TiedPlayer == BlasterPlayerState ? "(YOU)" : "";
						InfoTextString.Append(FString::Printf(TEXT("%s%s\n"), *TiedPlayer->GetPlayerName(), *strIfLocal));
					}
				}
...
void ABlasterPlayerController::ClientJoinMidgame_Implementation(FName StateOfMatch, float Warmup, float Match, float Cooldown, float StartingTime)
{
	...
	UE_LOG(LogTemp, Warning, TEXT("Controller::ClientJoinMidgame complete and LevelStartingTime = %f"), LevelStartingTime);
...
void ABlasterPlayerController::OnPossess(APawn* InPawn)
{
	...
	if (BlasterCharacter)
	{
		...
		//temprary
		SetHUDCarriedAmmo(120);
		SetHUDWeaponAmmo(12);
	}
}

void ABlasterPlayerController::ClientPlayEliminationSound_Implementation()
{
	if (EliminationSound)
	{
		UGameplayStatics::PlaySound2D(this, EliminationSound, 8.0f);
	}
}
